...
function initHighlight () {
  ...
  forEach(document.querySelectorAll('.highlight > .chroma'), $chroma => {
    ...
    if ($codeElements.length) {
      ...
      // CUSTOM: Replace with fix for double click issue -  $title.addEventListener('click', () => {
      // CUSTOM: Replace with fix for double click issue -    const content = $chroma.getElementsByClassName('table-wrapper')[0];
      // CUSTOM: Replace with fix for double click issue -    if ($chroma.classList.contains('open')) {
      // CUSTOM: Replace with fix for double click issue -      // Collapse the content
      // CUSTOM: Replace with fix for double click issue -      content.style.maxHeight = '0px';
      // CUSTOM: Replace with fix for double click issue -    } else {
      // CUSTOM: Replace with fix for double click issue -      // Expand the content to its full height
      // CUSTOM: Replace with fix for double click issue -      content.style.maxHeight = content.scrollHeight + 'px';
      // CUSTOM: Replace with fix for double click issue -    }
      // CUSTOM: Replace with fix for double click issue -    $chroma.classList.toggle('open');
      // CUSTOM: Replace with fix for double click issue -  }, false);
      ...
      // CUSTOM: replace with logic to handle open close better : if (window.config.code.maxShownLines < 0 || code.split('\n').length < window.config.code.maxShownLines + 2) $chroma.classList.add('open')
      const numberOfLines = code.split('\n').length;
      const content = $chroma.getElementsByClassName('table-wrapper')[0];

      // Set maxHeight based on maxShownLines
      // if (numberOfLines > window.config.code.maxShownLines) {
      //   // Start in a collapsed state, showing only maxShownLines
      //   // content.style.maxHeight = (window.config.code.maxShownLines * lineHeight) + 'px'; // assuming a certain lineHeight
      //   // code.split('\n').length < window.config.code.maxShownLines + 2);
      //   $chroma.classList.add('open');
      //   content.style.maxHeight = (window.config.code.maxShownLines * lineHeight) + 'px'; // assuming a certain lineHeight

      // } else {
      //   // Less lines than maxShownLines, start expanded
      //   // code.split('\n').length < window.config.code.maxShownLines + 2);
      //   $chroma.classList.add('open');
      // }

      if (numberOfLines <= window.config.code.maxShownLines) {
        // If the number of lines is less than or equal to maxShownLines, start expanded
        $chroma.classList.add('open');
        content.style.maxHeight = content.scrollHeight + 'px';
      } else {
        // If the number of lines is more than maxShownLines, start collapsed
        // You can set a specific maxHeight here to show some lines, or leave it as default
        content.style.maxHeight = (window.config.code.maxShownLines * lineHeight) + 'px';
      }
